...
create_folders(){
    ...
    then
        mkdir "$DATA_PATH"
        ln -s "$DATA_PATH" "/teamspeak/save" > /dev/null 2>&1
    fi
...
create_links(){
    ln -s "$DATA_PATH/ts3server.sqlitedb" /teamspeak/ts3server.sqlitedb > /dev/null 2>&1
    ln -s "$DATA_PATH/query_ip_whitelist.txt" /teamspeak/query_ip_whitelist.txt > /dev/null 2>&1
    ln -s "$DATA_PATH/query_ip_blacklist.txt" /teamspeak/query_ip_blacklist.txt > /dev/null 2>&1

    if ! [ -e "/teamspeak/logs" ]
    then
        ln -s "$DATA_PATH/logs" /teamspeak/logs > /dev/null 2>&1
    fi

    if ! [ -e "/teamspeak/files" ]
    then
        ln -s "$DATA_PATH/files" /teamspeak/files > /dev/null 2>&1
    fi
    ...
    if [ -e "$DATA_PATH/licensekey.dat" ]
    then
        ln -s "$DATA_PATH/licensekey.dat" /teamspeak/licensekey.dat > /dev/null 2>&1
    fi
}
